...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        transform = glm::rotate(transform, (float)glm::radians(0.050f), glm::vec3(0.0f, 0.0f, 1.0f)); 
        ourShader.updateMixValue(mixValue); 
        ourShader.updateMat4(transform);

        float ratio = sin(glfwGetTime());
        ourShader.updateRatio("ratio", ratio);
